...
(
  `id_forum_user` int NOT NULL AUTO_INCREMENT,
  `comment` varchar(200) DEFAULT NULL,
  `date` datetime DEFAULT NULL,
  `user_id` int DEFAULT NULL,
  `forum_id` int DEFAULT NULL,
  PRIMARY KEY (`id_forum_user`),
  KEY `id_user_idx` (`user_id`),
  KEY `id_forum_idx` (`forum_id`),
  CONSTRAINT `id_forum` FOREIGN KEY (`forum_id`) REFERENCES `topic` (`id_forum`) ON DELETE CASCADE ON UPDATE CASCADE,
